...
stats::stats(PNG & im){
    ...
    for (unsigned int i = 0; i < im.width(); i++) {
        sumHueX.push_back(vector<double>(im.height()));
        sumHueY.push_back(vector<double>(im.height()));
        sumLum.push_back(vector<double>(im.height()));
        sumSat.push_back(vector<double>(im.height()));
        hist.push_back(vector<vector<int>>(im.height()));
    }

    hist[0][0] = origHist;
    sumHueX[0][0] = cos(origin->h * PI / 180);
    sumHueY[0][0] = sin(origin->h * PI / 180);
    sumLum[0][0] = origin->l;
    sumSat[0][0] = origin->s;
    ...
    for (unsigned int row = 1; row < im.height(); row++) {
        ...
        lst[(int)px->h / 10]++;
        hist[0][row] = lst;
        sumHueX[0][row] = sumHueX[0][row-1] + cos(px->h * PI / 180);
        sumHueY[0][row] = sumHueY[0][row-1] + sin(px->h * PI / 180);
    ...
    for (unsigned int col = 1; col < im.width(); col++) {
        HSLAPixel* px = im.getPixel(col,0);
        vector<int> lst;
        for (int i = 0; i < 36; i++)
            lst.push_back(hist[col-1][0][i]);
        lst[(int)px->h / 10]++;
        hist[col][0] = lst;
        sumHueX[col][0] = sumHueX[col-1][0] + cos(px->h * PI / 180);
        sumHueY[col][0] = sumHueY[col-1][0] + sin(px->h * PI / 180);
    ...
    for (unsigned int row = 1; row < im.height(); row++) {
        for (unsigned int col = 1; col < im.width(); col++) {
            ...
            lst[(int)px->h / 10]++;
            hist[col][row] = lst;
            sumHueX[col][row] = sumHueX[col][row-1] + sumHueX[col-1][row] - sumHueX[col-1][row-1] + cos(px->h * PI / 180);
            sumHueY[col][row] = sumHueY[col][row-1] + sumHueY[col-1][row] - sumHueY[col-1][row-1] + sin(px->h * PI / 180);
...
HSLAPixel stats::getAvg(pair<int,int> ul, pair<int,int> lr){
    ...
    double hue = atan2( avghy, avghx )*180.0/PI;
    while ( hue < 0 ) hue += 360;
